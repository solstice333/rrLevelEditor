...
int Timer::delayFrame(int fps) {
	...
	if (this->getTicks() < 1000 / fps) SDL_Delay(
			delay = 1000 / fps - this->getTicks());
...
void Particle::show(SDL_Rect* camera) {
	if (type != NULL) applySurface(p.x - camera->x, p.y - camera->y, *type,
			screen);

	if (frame % 2 == 0 && p4 != NULL) applySurface(p.x - camera->x,
			p.y - camera->y, *p4, screen);
...
void StringInput::handleInput(SDL_Event& event) {
	if (event.type == SDL_KEYDOWN) {
		...
		if (str.length() <= 16) {
			if (event.key.keysym.unicode == (Uint16) ' ') str +=
					...
					|| (event.key.keysym.unicode >= (Uint16) 'a'
							&& event.key.keysym.unicode <= (Uint16) 'z')) str +=
					(char) event.key.keysym.unicode;
		}
...
void applySurfaceMiddle(Surface& source, SDL_Surface* destination,
	...
	applySurface((destination->w - source.getSDL_Surface()->w) / 2,
			(destination->h - source.getSDL_Surface()->h) / 2, source,
			destination, clip);
}
